...
const EXAM_TYPE_OPTIONS = [
  ...
  { key: '04_학평', label: '04월 학력평가' },
  { key: '05_학평', label: '05월 학력평가' },
  { key: '06_모평', label: '06월 모의평가' },
  { key: '07_학평', label: '07월 학력평가' },
  { key: '09_모평', label: '09월 모의평가' },
...
// 시험 이름 생성
function getExamName(examType: string, year: number, month: string, region: string | null): string {
  if (examType === '모평') {
    return `${year}학년도 ${month}월 모의평가`;
  } else {
    const regionText = region ? ` (${region})` : '';
    return `${year}년 ${month}월 고3 학력평가${regionText}`;
  }
}

export function G3Page() {
  ...
  const examGroups = useMemo(() => {
    const g3Exams = examsData.exams.filter(
      (exam: Exam) => exam.grade === '고3' && (exam.examType === '모평' || exam.examType === '학평')
    );

    const groups: Record<string, ExamGroup> = {};

    g3Exams.forEach((exam: Exam) => {
      const regionKey = exam.region || 'NA';
      const groupKey = `${exam.year}_${exam.month}_${exam.examType}_${regionKey}`;

      if (!groups[groupKey]) {
        groups[groupKey] = {
          id: `${exam.year}_${exam.month}_${exam.examType}_고3_${regionKey}`,
          name: getExamName(exam.examType, exam.year, exam.month, exam.region),
          year: exam.year,
          month: exam.month,
  ...
  const filteredGroups = useMemo(() => {
    ...
    if (selectedExamTypes.size > 0) {
      result = result.filter(group => {
        const groupKey = `${group.month}_${group.examType}`;
    ...
    <div className="w-full h-full flex flex-col overflow-hidden">
      ...
      <div className="h-14 border-b border-[var(--border)] flex items-center justify-between px-4 shrink-0 bg-white">
        <div className="flex items-end gap-2">
          <h1 className="text-lg font-semibold leading-none text-[var(--foreground)]">고3 모의고사</h1>
