...
interface OneProblemSolverDialogProps {
  open: boolean;
...
export function OneProblemSolverDialog({
  open,
  onOpenChange,
  mode,
  title,
  ...
  const handleNextProblem = async () => {
    // 현재 과목 유지
    const currentSubject = problem?.subjectId;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md gap-1 max-h-[calc(100dvh-2rem)] overflow-y-auto" showCloseButton={false}>
        <div>
        ...
        <div className="mt-1 min-h-[400px] relative">
          {/* 채점 애니메이션 */}
          {/* 이미지 프리로드 (숨김) */}
                ...
                return (
                  <div className="mb-3 space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-gray-600">{year}년 {grade} {month}월 {examType}</span>
                ...
                <div className="flex justify-end mb-3">
                  ...
                  >
                    ...
                    <div className="relative w-8 h-[18px] rounded-full bg-gray-200 transition-colors">
                      <div
                        className={`absolute top-[2px] w-[14px] h-[14px] rounded-full bg-white shadow-sm transition-all ${showProblemInResult ? 'left-[2px]' : 'left-[14px]'
                          }`}
                      />
                    </div>
