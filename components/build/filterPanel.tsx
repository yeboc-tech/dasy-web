...
interface FilterPanelProps {
  ...
  // null = no problems, all enabled; 'economy' or 'tonghapsahoe' = only that subject enabled
  lockedSubject?: 'economy' | 'tonghapsahoe' | null;
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="flex gap-2">
          ...
          <Button
            ...
            variant="outline"
            disabled={lockedSubject === 'economy'}
            ...
            } ${lockedSubject === 'economy' ? 'opacity-60 cursor-not-allowed' : ''}`}
          >
            통합사회 2
          </Button>
          {/* 경제 */}
          <Button
            onClick={() => onMainSubjectToggle('economy')}
            variant="outline"
            disabled={lockedSubject === 'tonghapsahoe'}
            className={`rounded-full px-6 py-2 text-sm font-medium transition-all ${
              selectedMainSubjects.includes('economy')
                ? 'border-black text-black bg-gray-100'
                : 'bg-white text-black border-gray-300 hover:bg-gray-50'
            } ${lockedSubject === 'tonghapsahoe' ? 'opacity-60 cursor-not-allowed' : ''}`}
          >
            경제
          </Button>
        </div>
